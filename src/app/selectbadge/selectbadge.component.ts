...
export class SelectBadgeComponent implements OnInit {
  ...
  public uploader: FileUploader = new FileUploader({ url: "http://http://172.16.18.189:3000/api/upload", itemAlias: 'photo' });
  ...
  ngOnInit() {
    ...
    var count = 0;
    var ids = this.http.get("http://http://172.16.18.189:3000/getIDS");
    ids.subscribe(val => {
      console.log(val);
  ...
  loadBadges(): void {
    ...
    var ids = this.http.get("http://http://172.16.18.189:3000/getUserIDS");
    ids.subscribe(val => {
      console.log(val);
      var temp = Object.values(val);
      temp.forEach(pic => {
        this.UserPictures.push("http://http://172.16.18.189:3000/picture/" + pic);
      })
    })
  };
  ...
  deleteBadge(index: number): void {
    if (confirm("Are You sure you want to delete?")) {
      ...
      var x = this.UserPictures[index].split("http://http://172.16.18.189:3000/picture/");
      var deleted = this.http.post("http://http://172.16.18.189:3000/deleteUserBadge/", { "id": x[1] });
  ...
  let obs = this.http.post("http://http://172.16.18.189:3000/api/upload", { "file": this.fileList });
    obs.subscribe(data => {
